...
| `test` | `passing` / `failing`, or `retired` | — | **the test runner**, from the exit code, when `command:` is set (ADR-0010). Author-written only for manual tests, which must carry `last_verified:` and go stale |
...
- **`retired` is terminal, and it is the only removal.** `TESTING.md`: acceptance tests are *"never removed, only deprecated"*, and the same holds for any test whose subject has been deleted — leaving it `passing` makes it claim to verify something that is gone, and deleting it is forbidden by `LIFECYCLE.md`.
...
- **It rests at `active`, and the verdict is `mark:`, never status** (`TAXONOMY.md`). Walking one writes `mark:`, `verdict_date:` and `verdict_reason:` and does not touch `status:`. This is the load-bearing sentence, and it has three deliberate consequences:
  - The runner-only rule does not engage, because the note is not at `passing`/`failing`. A person's judgement lives in a field that rule says nothing about.
...
- **Adding a `command:` is how it becomes automated**, and that is the point of the type being shared. From that moment the runner owns its status like any other executable test, and a `passing` covering test settles the acceptance test that names it in `covered_by:` — so automating a check discharges it instead of leaving the walk owed.
...
The type was introduced so that a check's human verdict could not collide with the machinery a test carries — the runner-only rule, the review gate and the `Run` obligation. That worked, and it cost the thing that turned out to matter more: **a check could not be automated.** A manual test becomes automated by adding `command:`; a check had no such path, so the 200-odd checks whose bodies already named a covering automated test still blocked releases waiting for a person.

...
| `check` | `retired` | — | agent or human; the **verdict** is `mark:`, never the status (see `[[check]]`) |
| `risk` | `closed` | — | agent |
...
- Typical transitions:
  - `draft` → `active` (the check is real and part of the walk)
  - `active` → `retired` (the surface it describes is gone, or the check was folded into another)
- **`retired` is terminal, and it is the only removal.** `TESTING.md`: acceptance checks are *"never removed, only deprecated"*. Deleting a check destroys the record that it was ever walked; retiring it keeps the verdict and takes it out of the gate.
- **The verdict is `mark:`, not status** (`TAXONOMY.md`). This is the load-bearing sentence of the type. Walking a check writes `mark:`, `verdict_date:` and `verdict_reason:` and never touches `status:`, which has three consequences, all deliberate:
  - The runner-only rule does not engage. It governs `[[test]]` statuses (`passing`/`failing`) written from an exit code; a check's verdict is a person's judgement by design, and it lives in a field that rule says nothing about.
  - The independent-review gate does not engage. `QUALITY.md` gates a `TST-*` reaching `passing`; a check never reaches `passing`. The review of a check *is* the walk.
  - No status value has to mean "failed". `!` does, in `mark:`, without adding a seventh word to a vocabulary ADR-0008 spent a measurement collapsing.
- A check carries no completeness gate of its own: nothing is blocked by a check being `draft`, and a `retired` check with an old verdict is a record, not an omission.

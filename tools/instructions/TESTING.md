...
- **`level:` is the whole distinction, and it is a spectrum rather than a boundary.** A `unit` test is a pytest module; an `acceptance` test is a thing a person walks. The same note can move between them, which is the point: **adding a `command:` to an acceptance test is how it becomes automated**, and from that moment the runner owns its status like any other executable test.
- **What used to be a type boundary is now a status one, and it is still load-bearing.** An acceptance test rests at `active`, so the runner-only rule (`STATUSES.md` `[[test]]`), the independent-review gate (`QUALITY.md`) and the `Run` obligation — keyed on `passing`, `passing` and `ready` respectively — never engage. **The review of an acceptance test is the walk.** A suite is hundreds of rows that re-arm on every overlapping change, and counting them individually is a badge that never empties.
- **A `passing` test named in an acceptance test's `covered_by:` settles it**, with no human mark. That is what the shared type buys: before it, automating a check bought nothing and the walk stayed owed.

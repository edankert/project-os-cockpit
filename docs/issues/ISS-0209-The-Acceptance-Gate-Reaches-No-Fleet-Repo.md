...
**Correction 3 — `VERIFY-ACCEPTANCE` is a warning until 2026-11-20, and it fires ten times, not six.** This note counts *"six true `VERIFY-ACCEPTANCE` findings (FEAT-0025 against TST-0028..0033)"*. Upstream reports **ten** against `your-sudoku` — those six plus `FEAT-0028` against `TST-0018..0021` — and `your-trainer` reports **19**. All of them are warnings inside upstream's grandfather window ending **2026-11-20**. So the gate is installed and reporting today, and *blocking* is a second date. The "done when" box above reads as one event and is two.

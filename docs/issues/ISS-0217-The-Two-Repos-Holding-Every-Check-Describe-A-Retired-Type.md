...
- [ ] Run `tools/scripts/sync-project-os.sh` for `TESTING.md` and `SCHEMAS.md` into `your-trainer` and `your-sudoku`, and diff before committing — these repos may carry deliberate local content in them (`your-trainer`'s tier definitions are its own and are cited by [[DES-0012]] D3).
- [ ] Confirm no `check.md` template remains in either repo's `docs/__templates__/`.
- [ ] Re-measure and record, so the next reader can see whether the fleet's instruction drift is closing or growing — the same closing condition [[ISS-0209]] carries.

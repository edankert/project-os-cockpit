...
- [ ] A design artifact is committed under `docs/designs/` beside its note and linked via `design:` from every note it specifies — evidence: <path>
- [ ] Each revision is a commit carrying its reason, **and** appears in the note's `## Revisions` log — evidence: <git log + the note>

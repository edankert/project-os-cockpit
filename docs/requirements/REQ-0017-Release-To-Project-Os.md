...
## Statement
The canonical project-os-cockpit source SHALL continue to live in its own repo (locally at `~/Dev/repos/project-os-cockpit`, GitHub: `edankert/project-os-cockpit`). A vendored copy SHALL be maintained at `~/Dev/repos/project-os/tools/cockpit/`, populated by a one-way sync from the canonical repo. Downstream project-os consumers receive the cockpit via their existing `sync-project-os.sh` flow — no separate install step required.
...
project-os is the meta repo — it ships templates and conventions but has no project-os *instance* notes (no FEAT-/TASK-/REQ-) to render. project-os-cockpit's development environment requires a consumer repo with real instance notes; this repo is exactly that. Folding project-os-cockpit into project-os would remove the only place where the cockpit can be dogfooded against real content.
...
- Installs the synced project-os-cockpit source into that venv if not already installed (`pip install -e .`).
...
## Acceptance Criteria
- Running `tools/scripts/release-to-project-os.sh` from the project-os-cockpit repo populates `~/Dev/repos/project-os/tools/cockpit/` with the source set above, plus `CANONICAL_SHA` and `CANONICAL_DATE` markers.
- Running with a dirty project-os-cockpit working tree fails fast with a clear error.
- Running when the project-os copy has uncommitted local edits fails fast with a clear error pointing at `git status`.
- `project-os/tools/cockpit/run.sh <docs-path>` from inside any downstream project starts project-os-cockpit against the given docs path, after a one-time venv bootstrap.
- LIFECYCLE.md (or equivalent) carries a rule that obligates the agent to run the sync as part of close-out for any CHG that touches project-os-cockpit source.
...
## Rationale
Two sources of truth become unmaintainable when sync is a human-attention task. With LLM-managed close-out, "always run the sync after a touching-project-os-cockpit CHG" is reliable — humans skip steps; agents under a rule do not. This pattern lets us:

- Keep project-os-cockpit's own dogfood corpus (this repo) as the dev environment.

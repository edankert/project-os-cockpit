...
id: REQ-0017
aliases: ["REQ-0017"]
title: "Release docs-server into project-os via one-way sync"
status: approved
...
# REQ-0017 — Release docs-server into project-os via one-way sync

## Statement
The canonical docs-server source SHALL continue to live in this repo (`~/Dev/repos/docs-server`). A vendored copy SHALL be maintained at `~/Dev/repos/project-os/tools/docs-server/`, populated by a one-way sync from the canonical repo. Downstream project-os consumers receive docs-server via their existing `sync-project-os.sh` flow — no separate install step required.
...
`project-os/tools/docs-server/run.sh` SHALL be a tiny wrapper that:

- Ensures a project-local Python venv exists at `tools/docs-server/.venv/` (or reuses one).
- Installs the synced docs-server source into that venv if not already installed (`pip install -e .`).
- Execs `python -m docs_server "$@"` so all CLI flags pass through.

The wrapper SHALL be ~10 lines, no exotic shell. Downstream projects invoke it as `tools/docs-server/run.sh docs --bind 0.0.0.0 --port 8765`.

### LLM-managed sync trigger
A rule SHALL be added to `tools/instructions/LIFECYCLE.md` (or an adjacent rules file) stating: when a CHG note merges that touches `src/docs_server/` or `pyproject.toml`, the agent's close-out routine MUST run the sync script and commit the result to project-os.
...
## Acceptance Criteria
- Running `tools/scripts/release-to-project-os.sh` from the docs-server repo populates `~/Dev/repos/project-os/tools/docs-server/` with the source set above, plus `CANONICAL_SHA` and `CANONICAL_DATE` markers.

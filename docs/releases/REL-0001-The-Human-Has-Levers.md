...
**That pass produced a finding of its own, and it is why the last two stay open.** The Electron shell had been running since 2026-08-09 19:46 — **1 day 23 hours** — so its renderer predates every session that touched this code. 1.10.1's third clause looked wrong (agent chips on notes nobody had touched) and was *not* filed as a defect, because a two-day-old renderer is exactly what produces that shape. Filed instead as [[ISS-0140]]: **nothing says the shell is stale**, which is the sidecar trap of two days ago wearing different clothes, and which has now cost a false observation twice.

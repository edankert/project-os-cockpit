...
**Step 5/6 — the tier gate.** 27 Tier 1 and 7 Tier 2 items. **16 were walked on 2026-08-10** and carry their evidence in the suite; **17 more on 2026-08-11 and one unticked**, so **32 of 34 are settled — 31 ticked, 1 reconciled — and 2 remain** — the gate is still red.
...
**That pass produced a finding of its own, and it is why the last two stay open.** The Electron shell had been running since 2026-08-09 19:46 — **1 day 23 hours** — so its renderer predates every session that touched this code. 1.10.1's third clause looked wrong (agent chips on notes nobody had touched) and was *not* filed as a defect, because a two-day-old renderer is exactly what produces that shape. Filed instead as [[ISS-0140]]: **nothing says the shell is stale**, which is the sidecar trap of two days ago wearing different clothes, and which had by then cost a false observation twice. **Fixed the same day** — `GET /api/cockpit/runtime` compares the running process against the code on disk, and the Verification card carries one line when either the sidecar or the window is behind. It reports and never reloads: a window reloaded under someone mid-session is worse than the staleness it fixes.

**The two that remain — 1.9.1 and 1.10.1 — need an agent CLI started inside the app's own terminal.** That spends real tokens in the principal's session and is his to start, and it should follow a restart rather than precede one.
...
**One check was cut rather than walked.** 1.5.2 asked for a Changes band on the overview; [[FEAT-0052]] retired that tile on 2026-07-30, **eleven days before this suite was written**. It is reconciled `- [~]` with that history, because it describes a surface that was removed by decision — a finding about the checklist, not the product.

...
**Independence.** Four rounds, one continuous reviewer session that has never seen the author's reasoning trace: I worked from the notes, the diffs, and what I could execute. Same model family as the author, recorded in `reviewed_by` as provenance ([[ADR-0013]]: clean context is the mechanism, family is not the gate). Rounds 1–3 returned `changes-requested` and are preserved above rather than rewritten, because the record of what was wrong is the evidence that this pass did anything.

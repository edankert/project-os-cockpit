...
## Independent review, round 3 — 2026-07-28 (`model:claude-opus-5`, commits dbe8ec3 + 046108c)

Verdict stays `changes-requested`, on one branch. `491 passed`, `validate-docs: OK`, tree clean.

**Round-2 finding 1 is fixed, and I verified it independently across the whole design vocabulary rather than on the two cases the tests cover.** Through the endpoint: `implemented` → accept → stays `implemented` (`status: None` in the response) with `review_verdict` and `design_revision` still written; `superseded` and `cancelled` likewise decline the move; `proposed` still advances to `accepted`. The guard did not eat the feature. Findings 2, 3, 5 are fixed and I confirmed each on the wire, not by reading: `subject_type` is `"design"` for a subject-without-revision row, a document-level comment (`region: ""`) posted by "Request changes" is accepted and lands in the note's `## Review` section as `**(document)** · date · user:edwin — text`, and the two dirty conditions are now separate branches with different tenses.

**1 (blocking, and the mirror of round 2's finding 1). Reject still cancels a design that shipped.** The rank guard is accept-only — `backwards = bool(accept and ...)` — so `implemented` + Reject writes `status: "cancelled"`, verified through the endpoint. The rank table cannot see this even in principle: it ranks `cancelled`/`superseded` at 5, *above* `implemented` at 4, so cancelling a shipped design reads as a forward move. The docstring on `buildDesignReviewView` now says *"rejecting goes through the design endpoint so a built design is not cancelled by a status posted from the client"* — true about the mechanism, false about the consequence: the client no longer posts the status and the design is cancelled anyway. `cancelled` on a design whose code shipped is untrue in exactly the way `accepted` was, it is reachable from the live DES-0002 row, and the argument that carried round 2 ("the verdict is the honest part; only the status move is declined") applies unchanged. Fix is the same shape: decline the decline-transition from `implemented` too, recording `review_verdict: rejected` with no status move.

**2. The loopback test catches deletion but not "guard after the side effect".** Your fix is real — I deleted `if not self._require_loopback(): return` from `_serve_design_offer_review` and the test failed, which neither previous version did. Then I moved the guard to sit *after* `review_store.add(...)`: the test passes, and a LAN client gets `403 {"ok": false, "error": "mutations are loopback-only"}` while the ledger row is written to `.cockpit/review-requests.json`. Verified with a real request through the handler with `_is_loopback` forced false. The rule you wrote into ISS-0056 is right and one clause short: a guard test must also assert the guarded side effect did not happen. Here that is one line — re-read the queue after the 403 and assert it is empty.

**3. `buildDesignFrame` still gates the historical render on the working copy, one layer below the fix.** The outer gate is now `if (!d.has_asset && !detail.at_revision)`, but `buildDesignFrame` (renderer.ts:2868) keeps its own unconditional `if (!d.has_asset) { … "Artifact not found" … return wrap; }` before it ever builds the iframe. So a design whose artifact was deleted after being offered still does not render at its reviewed revision — it just shows a different message. `design-asset-at` reads from git and would serve it fine. The new test asserts the outer substring and cannot see the inner return. Fix: `if (!d.has_asset && !atSha)` in `buildDesignFrame`, mirroring the caller.

**4. `_DESIGN_STATUS_RANK` is fine as a local ordering; its membership is what needs pinning.** I checked the ISS-0042/ISS-0023 hypothesis and it does not hold the way you feared: `statuses.py` owns a *vocabulary* and a *band categorisation* (`BANDS` is a dict including `blocked` and `reference`), not a lifecycle order, so there is no existing ordering this duplicates — a band-based rule would have had to invent one too. The table currently matches `ALLOWED_STATUS["design"]` exactly, which I verified. The real exposure is drift with a silent fail-open: an unranked status returns `0` from `.get(current, 0)`, is never "backwards", and gets demoted. Add `assert set(_DESIGN_STATUS_RANK) == ALLOWED_STATUS["design"]` so a new design status forces a rank decision instead of quietly reopening finding 1.

**Confirmations you asked for.** The layout rework (046108c) does not change any round-2 finding except by making the header/comment conventions consistent, which is an improvement, and none of my findings were against the layout. Per-region verdicts (ISS-0055) and the missing `record_dispatch`/SSE publish both remain non-blocking. **The note-prose staleness boundary belongs in its own issue, not here**: `design_revisions_payload` is artifact-scoped by design (TASK-0216/TASK-0220 own it), the gap affects the `proposed` intake path identically, and folding it in would reopen a scope this task deliberately held to the ledger. File it against FEAT-0042 linked to TASK-0220.

## Independent review, round 4 — 2026-07-28 (`model:claude-opus-5`, commit 1254960) — **approved**

`approved`. All four round-3 findings are fixed, and I verified each independently rather than from the report. `496 passed`, `validate-docs: OK`, tree clean. Nothing outstanding changes behaviour on any path I could construct; the four notes below are comment- and test-level, and none of them blocks close-out.

**Verified on the wire, not read.** Reject across the whole vocabulary: `implemented` stays `implemented` with `review_verdict: rejected` recorded (`status: None` in the response), `superseded` and `cancelled` likewise, `proposed` still cancels — so the refusal is about settled statuses, not about rejection. Accept keeps its round-3 behaviour. The loopback guard is the first statement in the shipped handler, and my round-3 mutation (guard moved after `review_store.add`) now **fails** the test on the written-row assertion. `buildDesignFrame`'s inner return is `if (!d.has_asset && !atSha)`, so a deleted artifact renders from git at its reviewed revision. The rank table fails closed and its membership is pinned to `ALLOWED_STATUS["design"]`.

**On "is re-reading the queue sufficient?" — no, and here is the case it misses.** I moved the guard to sit immediately *before* `review_store.add(...)`, i.e. after every read branch. The test passes (403 on the happy path, no ledger row) while a LAN client can still enumerate the design register: `403` for a real id versus `404 unknown design` for a fake one versus `400 id is required`, plus a `designs_payload` scan and two `git` subprocesses per probe. Demonstrated against a fixture with `_is_loopback` forced false. The queue cannot reveal either effect, because neither is a write. The third clause for ISS-0056's rule: **the refusal must also pre-empt the endpoint's other branches** — assert that a non-loopback POST with an *unknown* id is `403`, not `404`. That one assertion pins the guard to the top of the handler, which is where it is.

**The "intention rather than code" hunt you asked for — one instance is inside this round's own fix.** `_DESIGN_STATUS_RANK`'s comment says it is *"used to refuse a move that would rewrite history"*. It no longer refuses anything: I replaced the whole `backwards = bool(...)` expression with `False` and **all 496 tests pass**. The ordering cannot fire, by construction — accept's candidate is `accepted` (rank 3) and every status ranking above 3 is in `_DESIGN_SETTLED`, which is checked first; reject never evaluates it. What the table is still load-bearing for is *membership* (`known = current in _DESIGN_STATUS_RANK`), the fail-closed check. Either delete the comparison and keep the set as what it now is — the known-status set — or keep the ranks and say they are decoration. As found: the ranks read as the guard, and the guard is `_DESIGN_SETTLED`.

**Two smaller instances of the same family.** `review.py`'s module docstring still asserts *"filing one records a dispatch entry too, so the provenance chain from prompt → session → request stays intact"* — the offer path files without `record_dispatch`, which you deliberately deferred; deferring the code is fine, but the docstring should stop claiming the invariant. And in `_serve_design_offer_review` the four-line comment about the working copy and `dirty_at_offer` is attached to the `body=note,` argument, describing code that lives after the call.

**Standing confirmations.** Per-region verdicts (ISS-0055) and `record_dispatch`/SSE remain non-blocking. ISS-0057 is the right home for the artifact-only staleness boundary, and the question you added to it — that a naive "did the note change" check goes stale the moment a review appends to `## Review` — is the one that makes it a design problem rather than a one-line diff.

**Independence.** Four rounds, one continuous reviewer session that has never seen the author's reasoning trace: I worked from the notes, the diffs, and what I could execute. Same model family as the author, recorded in `reviewed_by` as provenance ([[project-os-dev#ADR-0013]]: clean context is the mechanism, family is not the gate). Rounds 1–3 returned `changes-requested` and are preserved above rather than rewritten, because the record of what was wrong is the evidence that this pass did anything.

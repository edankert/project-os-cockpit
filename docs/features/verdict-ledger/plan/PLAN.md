...
## Stage 1 — complete, 2026-08-19

FEAT-0133 through FEAT-0137 are `done`; REQ-0052..REQ-0056 are `implemented`. **The ledger is the only source in this repo**, which is what "not an overlay" means:

- 34 notes carry no verdict field — `LEDGER-FIELD` refuses one, *in a repo that keeps ledgers*.
- `apply_ledger` is the read path, `record_verdict` the write path, and the guard [[REQ-0055]] asked for exists because the field is gone: five different scalars written into a note's frontmatter, verdict does not move.
- `suite_at` reads all three historical shapes.
- The gate, the run list, the burndown and `tests_verified` are queries.
- Every payload names the platform its answer is about; `all` means the union, which fails closed.
- Upstream `project-os@ce789d7` carries the schema, so the next repo to migrate starts from the right template.

**What is deliberately not done**, and none of it is Stage 1: [[FEAT-0138]] (observed coverage), the fleet migration, and [[ISS-0220]] — the sealed-ledger check compares the working tree to `HEAD`, so an edit that is committed passes forever. That gap is asserted by a test so it cannot quietly stop being true.

**Three defects were found by doing this and none was in scope**: [[ISS-0219]] (two checks claiming one address, red on `main` before this started), [[ISS-0221]] (`suite_at` returned `None` at HEAD because a prefix filter never followed ADR-0031's renumber, so the release delta reported *no baseline* everywhere for a day), and the bidirectional template divergence between this repo and upstream that [[ISS-0217]] names only for the fleet.

...
- [ ] `contents.ids` renders — `REL-0001` currently reads *What shipped — 27 feature(s)* over an empty list
- [ ] Acceptance tests come from `tests_verified:`, resolved to their notes
- [ ] A release with an empty `tests_verified` says *not recorded*, and five of twelve are empty
- [ ] The gate section is ABSENT for a shipped release; its snapshot stands in its place
- [ ] The note's `## Known issues (shipping with)` section is surfaced as what it shipped with
- [ ] Artifacts are found by the `REL-####-` filename convention, blessed in [[ADR-0028]] rather than inferred

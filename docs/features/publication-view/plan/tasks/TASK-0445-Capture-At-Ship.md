...
- [x] Shipping asks which suite snapshot this release verified
- [x] `tests_verified:` is written from the answer, alongside any `TST-*` the release names
- [x] **Nothing is written unasked** — no auto-snapshot, no auto-copy
- [x] Declining leaves `tests_verified` empty and the page says *not recorded*, which is honest
- [x] Loopback-only, enumerated by the existing guard

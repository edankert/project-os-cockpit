...
18. **The stat tiles** ([[ISS-0063]], which this note declares in `verifies:`). Three assertions: `test_the_dead_stat_tiles_gained_a_destination` parametrised over Risks→`issues` and Tests→`review`, plus `test_the_reqs_tile_stays_dead_on_purpose`. Source-level over `buildStatTile` call sites, so a call site that drops its `navMode` argument fails — while setting `navMode = undefined` *inside* the function does not (disclosed under Adequacy). Cited by name in [[REQ-0025]] criterion 2 and [[PHASE-010]] exit criterion 3, and absent from this register for three rounds.

...
0. **Read the survey** ([[TASK-0414-The-Remote-Transport-Round]]) — done 2026-08-13, and it turned step 1 from a search into a comparison. Its source-verification list is step 1's first action.
1. **Decide the transport** ([[TASK-0405]]) before any remote UI: one authenticated connection, verified host identity, and an answer to *what authorises a mutation on a host with other accounts on it*. Three known shapes survive — a remote Unix socket forwarded over SSH, a loopback listener holding a per-connection key, or stdio. Two of the three are VS Code's, so the spike compares rather than invents, on a **Linux** host.
...
- How does a remote workspace's agent state reach the rail — polling multiplexed over one SSH connection, or a held-open subscription per remote workspace? The local poller exists specifically to avoid a live connection per workspace; the remote case inverts that trade and the answer belongs in [[TASK-0405]]. **This is not independent of the transport choice:** VS Code's socket mode — the strongest answer to the co-tenant problem — *disables connection multiplexing*, which is the mechanism this question was going to lean on. Answer both together.
- Does the remote install belong to the cockpit or to the user? VS Code installs, versions and can uninstall itself on the remote host; t3.code writes a launcher script. Whichever we choose, [[TASK-0411]] now requires that we can remove what we put there.

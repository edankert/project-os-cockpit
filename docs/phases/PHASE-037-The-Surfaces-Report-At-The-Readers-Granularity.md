...
`CLAUDE.md`'s rule is that a phase needs a goal statable without listing its parts, and exit criteria that are not *"the tasks are done"*. Both hold, and the common cause is sharper than the list of symptoms.
...
- **[[ISS-0254]]** — the [[ADR-0035]] guard enumerates function names and was evaded in three consecutive review rounds, each time by a route the list did not mention. It is widened again and the product is right; **completeness here is a property of the call graph and the test asks about text.** Filed with the repro rather than widened a fourth time, on this phase's own thesis: a rule that fires only on what somebody already thought of is a rule that cannot fire.
...
**Verdict: approved — the phase can close.** The sixth pass said it could close *"the moment a release item row stops calling itself automated"*, and it has: a manual check on `~release/<id>/<ITEM-ID>` now renders with no `is-automated` class, no command line and no control, which I established by executing `buildCheckRow` under a DOM shim rather than by reading its signature. All eight sixth-pass findings were acted on and seven are fully closed. Five findings below; none of them is a defect in the shipped product, and the two that matter are a guard weaker than the sentence beside it, with a compiling reproduction attached.

### The headline question: did fixing round six break anything

**No.** Test-function names were extracted per file at `c4413e3` and at `9784205` and the sets diffed: **nothing removed**, four added — `test_the_declarations_own_module_beats_a_bare_stem_match`, `test_two_classes_in_the_declarations_module_are_a_refusal` and `test_an_unattributable_run_says_so_on_stderr_too` in `tests/test_observed_coverage.py`, and `test_a_release_row_does_not_call_a_manual_check_automated` in `tests/test_release_held_back.py` — 1845 → **1849** functions across 112 files, suite 2072 → **2076**. Round two's failure mode has not recurred, and neither has round five's: the third parameter defaults to `true`, so `paintCheckList`'s `buildCheckRow(item, manual)` is byte-for-byte the same render it was before, confirmed by executing both forms rather than by reading the signature. The emitter was run in loops against temporary repos with the ledger entries counted: six identical passing runs → **1** entry; four consecutive failing runs → **0**; `pass`→`fail`→`pass`→`pass` → **3**; `pass` then five skipped runs → **2**; `pass` then five partial runs that never reach the test → **1**; six alternating pass/fail → **6**, one per transition. No unbounded growth anywhere. **Five of the six rounds introduced something while fixing; this one did not.**

### F1 verified by rendering, not by reading

`buildCheckRow` was lifted out of `desktop/dist/renderer/renderer.js` and executed under a DOM shim, four ways. **A manual check on a release item row** (`buildCheckRow(item, !item.command, false)`) emits `<div class="checks-row">` — **no** `is-automated`, **no** `checks-row-command`, no mark button, no `Retire`, and the only click handler left on the row is the id link. **A command-bearing check on the same surface** emits `is-automated` and a `checks-row-command` carrying **the real command**, still with no controls — a true statement rather than the fallback word. The `~checks` page is unchanged in both of its forms. Counted against the corpus: this repo holds **34** acceptance checks and **0** carry a `command:`, and `publication.release_item_payload` over all **142** features yields **67** rows across *originated* / *invalidated* / *in its areas*, every one of them command-less — so the 67 rows that printed the word `automated` now print nothing. Four mutants each fail a named guard: `buildCheckRow(item)`, `buildCheckRow(item, false)`, `buildCheckRow(item, !item.command, true)` and `buildCheckRow(item, true, false)`.

*(One correction to the sixth pass's own arithmetic, which does not touch the 67: it says the payload was built for **143** features. There are **142** — `index.notes_by_type("feature")` returns 142, there are 142 `FEAT-*.md` files, and no id appears twice.)*

### Finding 1 (medium) — a live mark control still reaches a release surface past the ADR-0035 guard, and the guard's own new comment says it cannot

The round-six repair replaced the `buildCheckRow` **spellings** in `forbidden` with a rule on the **argument**, and that rule is real. What it did not do is revisit the other five entries, which are still spellings — and the list names `markGateRow(`, a function deleted from the file, while omitting `checkMark(`, `markCheckRow(` and `paintCheckList(`, the live ones that open the same dialog.

**Constructed and compiled, not argued.** Two mutants planted in `buildReleaseItemPage`:

- `s.appendChild(checkMark(item));` — the mark button itself, whose click calls `markCheckRow` → `walkOneCheck` → `askForMark` → `POST /api/notes/mark-check`;
- a `Mark` button whose click handler is `void markCheckRow(item);`.

Each **typechecks clean** (`npx tsc --noEmit`, exit 0) and each leaves **all 18 tests in `tests/test_release_held_back.py` green**, including `test_no_write_path_to_a_check_appears_on_the_release_page`. That is [[ISS-0210]] restored on the page [[ADR-0035]] names, past the guard widened twice to prevent it.

**The product is correct and the claim about it is not.** An independent transitive scan of every call out of all nine release surfaces finds exactly one route to a check write — `buildReleaseItemPage` → `buildCheckRow` — and it is gated on `manual && controls` with `controls` hard-`false`. So exit criterion 1 holds. What is wider than the code is the new comment beside the rule: *"Every call inside a release surface must pass `controls` as a literal `false`, **which is the property — this surface offers no verdict control** — rather than a way of writing it."* Passing `controls: false` to one helper is not that property, and the two mutants above are the counter-example. This is the phase's signature defect one level up: the fix for *a rule stated wider than it is enforced* is itself stated wider than it is enforced.

### Finding 2 (medium-low) — the round-five defect is re-admissible past both guards with one comment line

`buildCheckRow(item, false, false)` satisfies the argument rule — three arguments, third is a literal `false` — and reinstates exactly what the sixth pass found: `is-automated` and the word `automated` on all 67 rows. It is caught today by one thing only, the whole-file substring `assert "buildCheckRow(item, !item.command, false)" in src`. Measured both ways: that call alone → **caught** by `test_a_release_row_does_not_call_a_manual_check_automated`; the same call with one comment line above it reading *"the shape is `buildCheckRow(item, !item.command, false)`"* → **both tests pass**.

The assertion is scanned over 470,000 characters **including comments**, which is the scoping defect the sixth pass filed its own F2 about, carried into the test written to answer it. It is not hypothetical for this file: `renderer.ts` line 7631 already carries a comment quoting `buildCheckRow(item, false)` — the *previous* round's assertion string — so the shape has been written here once already, by the same hand, in the same function.

### Finding 3 (low) — one `review_response:` says four gaps closed and three are

[[FEAT-0138]]'s sixth entry reads *"approved with four guard gaps recorded, **all four now closed**."* Three are. The third — *"Nothing in the record, and nothing on any surface, can tell a reader the check stopped being observed"* — is unchanged, and I measured it rather than inferring it: after a `pass` is observed, six consecutive runs in which the declaring test is present but unattributable leave the verdict at `mark='pass', method='automated'`, the exit code at **0**, the ledger at **one** entry, and stdout still ending *"nothing changed (0 check(s) observed passing)"*. What the commit closed is the finding's **headline** — the notice now also reaches stderr, and deleting that line fails `test_an_unattributable_run_says_so_on_stderr_too` — not its body. [[TASK-0543]]'s entry describes the same edit as *"NOT ATTRIBUTED on stderr"* and is exactly right, so two fields describing one change disagree about its size.

### Finding 4 (trivial) — two sentences in one comment, and an off-by-one

`tests/test_release_held_back.py:378` — *"so a **ninth** is a deliberate edit here"*. `expected` already holds **nine** names, so a new release surface would be a tenth. Four lines above it the same comment still carries the enumerated version's rationale — *"Named rather than pattern-matched, so renaming one into existence outside this list is a visible edit here"* — immediately above *"**Discovered, not enumerated.**"* The first sentence is false of the code beneath it: the set is pattern-discovered plus four named exceptions.

*(And an observation rather than a finding, on the defect list: the arithmetic is right for the first time in five rounds — heading **Eight**, list numbered 1–7, one unnumbered paragraph, 7 + 1 = 8 — but the *"seven in the product, one in the tooling"* split puts four `emit-coverage.py` defects in the product and the `coverage-declarations.py` one in the tooling, and both files sit in `tools/scripts/`. The count is now checkable against the list; the label is not checkable against anything.)*

### Finding 5 (trivial) — the caller-facing list does not carry the most visible change to a page

`CHG-20260821-Three-Silences-Get-A-Voice`'s *"Behaviour a caller can see"* lists four items, none of them the one a person would notice first: `~release/<id>/<ITEM-ID>`'s check rows stopped carrying a mark control and a `Retire` button. It is recorded in this phase note's defect list and inside two review sections, and nowhere a reader asking *what changed for a caller* would look.

### What survived refutation

- **All eight of the sixth pass's findings were acted on, and seven are fully closed.** F1 by render (above); F2's argument rule is real for `buildCheckRow` (Findings 1 and 2 are about what it does not reach); F3 and F4 by mutant — swapping the two tiers in `_resolve` fails **exactly** `test_the_declarations_own_module_beats_a_bare_stem_match`, and `if exact: return exact[0]` fails **exactly** `test_two_classes_in_the_declarations_module_are_a_refusal`, each being the only test in the file that catches its mutant; F5's arithmetic is right; F6's docstring now says `(passing, failing, stale, current, unattributable)` and the function returns five values; F7's stderr line is guarded; F8's two banners now differ, the `### Refuted` one naming refutations 1 and 2 as superseded and 3–5 as standing, and no longer quoting its neighbour's sentence.
- **The membership is exactly as claimed.** 97 notes carry `phase: [[PHASE-037…]]` — 38 task, 30 issue, 11 feature, 9 requirement, 5 change, 3 ADR-type, 1 design — with 48 `done`, 29 `fixed`, 9 `implemented`, 5 `merged`, 4 `accepted`, 1 `declined`, 1 `superseded`. **Zero non-terminal.** Both breakdowns sum to 97.
- **All five exit criteria re-verified independently, and none claims more than is true.** Criterion 1 by transitive call scan over all nine release surfaces; criterion 2's narrowed sentence is still the honest one; criterion 3 by `level: acceptance` selection in `acceptance.py`; criterion 4 by the server-side sorts in `cockpit.py` (surfaces by percentage incomplete descending, children incomplete-first, one `_is_incomplete` predicate); criterion 5 by the renderer emitting `N feature(s) held back · M check(s) no longer gating` from `gate.deselection`.
- **The `review_response:` chains are accurate, and *"six rounds"* is true of four notes rather than of the set.** Six segments on [[PHASE-037-The-Surfaces-Report-At-The-Readers-Granularity]], [[FEAT-0138]], [[REQ-0057]] and [[TASK-0543]]; **five** on `CHG-20260821-Coverage-Is-Observed-Not-Declared`, which the sixth pass approved with no findings, so nothing was owed; two on [[FEAT-0142]] and [[TASK-0576]], approved through the fourth pass. Each shortfall is correct rather than missing. One sentence inside them is not (Finding 3).

### Can this phase close

**Yes. Close it.** The release item row has stopped calling itself automated — established by executing the renderer, not by reading it — and that was the sixth pass's whole remaining condition. Every exit criterion is true of the shipped product, the membership is 97 terminal notes with nothing outstanding, the record's numbers reproduce, and for the first time in seven rounds **the repair introduced nothing**: no test deleted, four added, suite up by four, validator at zero errors, `--as-committed` green.

What remains is not a product defect and not a false claim on any surface. Findings 1 and 2 are a **guard weaker than the sentence beside it**, with a compiling reproduction attached, and they belong in an `ISS-*` rather than in a seventh loop — this phase already keeps a *"What is deliberately not closed"* section for exactly this shape, and a named reproduction filed there is worth more than another round. Findings 3, 4 and 5 are sentences.

The one thing I would not let close silently: **the ADR-0035 guard admits `checkMark(item)` and `markCheckRow(item)` on a release surface, and both compile.** That should be filed before the phase is put down, because the phase's own thesis is that a rule which cannot fire is the defect, and this is one.

...
- [x] **A mark rendered AS A MARK is a glyph**, guarded by `test_no_surface_brackets_a_raw_mark_rather_than_its_glyph` and `test_no_surface_renders_a_raw_mark_unbracketed_either`, which fail if a render site reads `mark` instead of going through `MARK_GLYPH`. [[FEAT-0126]], [[REQ-0045]] (`implemented`).
...
### Six defects found while building, each of them a rule that could not fire

The pattern this phase kept meeting, met **six** more times on its last day — five in the product and one in the tooling. *(This heading said "four" over a list of four-plus-one while the sentence under it said "three". Third arithmetic slip in one note, and the second a reviewer found by counting what the prose asserted.)*

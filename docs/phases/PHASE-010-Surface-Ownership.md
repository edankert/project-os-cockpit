...
- [x] The Risks and Tests stat tiles both navigate somewhere — evidence: `test_the_dead_stat_tiles_gained_a_destination`; manual run step 3 clicked both — Risks → `issues`, Tests → `~review`. Reqs stays inert by decision (see Out of Scope) and `test_the_reqs_tile_stays_dead_on_purpose` records that so it does not read as an oversight. **Both destinations changed on 2026-08-10** ([[TASK-0371]]): Tests points at the Tests view, Risks at the constraints view after [[ISS-0128]] moved the type. The guard was rewritten in the same pass — the old one asserted the mode *string*, which is why it kept passing for a commit while the Risks tile pointed at a pane with no risks in it

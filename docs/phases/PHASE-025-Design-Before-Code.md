...
- [~] A feature naming an unaccepted design cannot start — **reconciled: it warns rather than blocks.** `DESIGN-GATE` fires on a feature past the pending band whose design was never accepted, proven both ways and quiet on this corpus ([[TASK-0309]]). "Cannot start" was the criterion as written; a blocking gate on a judgment that cannot be automated gets cleared to unblock the build rather than because somebody looked, which is the same argument ACCEPT-STALE and independent review both settled on. Escalation is deferred until lived with ([[project-os-dev#ADR-0011]]'s path)

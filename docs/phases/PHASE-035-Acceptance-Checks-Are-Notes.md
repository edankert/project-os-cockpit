...
- [x] **No note in any repo carries `type: [[check]]`** — measured across all twelve, and the last two were `check.md` templates, removed. *Reconciled on the second clause: the reading code still accepts the retired type, deliberately, so that a repo which never held a check does not start failing on one it does not use.*
- [x] **The Tests obligation badge is unchanged in every repo** — `project-os-cockpit` 1 → 1, `your-sudoku` 0 → 0, `your-trainer` 5 → 5. *It moved to 3 in the pilot and nobody noticed until independent review measured it; the cause was a rename missed in `SUBJECT_FIELDS`, not the acceptance notes.*
- [x] **Parity through the reader in all three suites** — 34 / 56 / 579 notes, identical `mark:` distributions, `covers:` target sets and gate figures (0 / 56 / 60), and `your-trainer`'s twelve-tag delta unchanged. *The tag figures are 84, 87, 87, 111, 212, 268, 361, 536, 560, 560, 560, 560 — the ones written here were the ISS-0192 blocking counts, quoted from the wrong measurement.*
- [~] **A check is discharged by automating it** — demonstrated end to end on real notes (`test_a_passing_covering_test_settles_the_check`), including the three ways it must NOT settle. **Reconciled rather than ticked**: no check in the corpus is discharged this way yet, because `covered_by:` has no writer ([[TASK-0483-The-Covered-By-Action]]) and the 203 `your-trainer` annotations name JVM classes rather than test ids ([[TASK-0485-Backfill-Automation-From-The-Prose]]). The mechanism works; the corpus has not been given the data.
- [~] **One verification link** — done in this repo: `tests:` gone from all 81 features that carried it, the legacy fields gone from all 43 tests, the path fallback deleted, and the inversion proven across twelve repos (56 → 57, the one delta a true finding the old lookup was blind to). **Reconciled**: `task`, `issue` and `requirement` still carry `tests:` (248 live edges), and the other repos have not consolidated — the forward-field fallback covers them.
- [x] **An independent review of the result** — `changes-requested` on all four features, right on every blocking finding, and every one fixed. It caught a criterion I had ticked while false (the badge), a false-positive report that hid a second finding (81 features still carrying `tests:`), and an exemption hole it reproduced. [[TASK-0490-Independent-Review-Of-The-Merge]] records what changed.
...
- [~] **`your-trainer`'s obligation badge total does not increase at any point in this phase.** — **Not met, and the phase note contradicts itself here.** Measured: `your-trainer` 32 → 36, `your-sudoku` 7 → 12, this repo 29 → 31. **Every one of those rows is a FEATURE, and zero are checks.** The rise is [[TASK-0468-The-Considered-Obligation]] working exactly as this phase designed it — a feature in flight with no `acceptance_impact:` is asked whether its acceptance impact was considered, which is the obligation [[FEAT-0115-The-Sweep-Is-Continuous]] exists to create and which Edwin asked for in as many words. The guarantee that was actually at risk holds exactly: **0 of 669 checks reach any badge, any Needs-you group or any digest**, asserted on a corpus where every check is unwalked. The clause as written was about per-check flooding and was drafted as though the sweep obligation were free; it is not, and four features being asked one question is the price of the rule Edwin asked for.
  ...
  *Reconciled 2026-08-18, and the reconciliation is narrower than the criterion.* **The claim this criterion was written to protect holds exactly**: the merge migration moved 669 checks onto the test type and **not one reached a badge** — `your-trainer` 5 → 5, `your-sudoku` 0 → 0, this repo 1 → 1, measured per repo before and after. What did not hold is the criterion as literally worded, because the *acceptance-sweep* obligation this phase also built asks features a new question, and that is the design working rather than failing.

  It is marked reconciled rather than ticked because a criterion should not be quietly reworded into one that passes.
...
Nine features, thirty-one tasks, seven issues, four requirements and three ADRs — one of which superseded another inside the same phase, one day after it was accepted.

**What the phase set out to do and did:** the acceptance record left a single grammar-bearing document and became notes; the suite became a generated view with the same marks; the sweep became continuous; a release can be finished inside the cockpit. Then use found what the design had not — *a check could not be automated* — and the phase widened rather than spawning a successor.

**What that second half delivered:** one type. 669 checks across three repos are `[[test]]` notes at `level: acceptance`; no note in any of the twelve repos carries `type: [[check]]`; `covered_by:` reaches the gate so automating a walk discharges it; the verification link has one direction and one encoding; and both write paths exist — *Covered by* behind four refusals, promote-and-retire behind two.

**Two exit criteria are reconciled rather than ticked, and both reconciliations are the honest kind.** The badge criterion held for what it was written to protect — 669 notes, zero badges — and not as literally worded, because the acceptance-sweep obligation this same phase built asks features a new question. And no check in the fleet is discharged by automation *yet*: `your-trainer`'s 203 annotations name 54 JVM classes and not one names a test id, so the mechanism is proven and the corpus has not been given data it can act on. Writing 54 unrunnable test notes to make a number move would have been the opposite of what the requirement asks.

**Three things this phase taught that are worth more than what it shipped:**

1. **A false report hides the finding under it.** Two removals reported success having changed nothing — a regex needing a trailing newline the last frontmatter line does not have, and an edit whose script died before writing. Both printed a count. Independent review found both.
2. **A construction nothing asserts is one careless write from failing.** ADR-0031 rests on acceptance tests sitting at `active`; a rename missed in one table disarmed a rule two ADRs away and moved the badge 1 → 3 with nothing reporting it.
3. **Guards die with their fixtures.** `mark_check` refused every note in every repo after the migration, and nothing caught it, because every fixture still built the retired type.

...
**Six of nine hold as of 2026-08-19.** The three that do not are named, not softened.
...
- [ ] **A sealed ledger cannot be modified.** *Half true.* `LEDGER-SEALED` compares the working tree to `HEAD`, so an edit that is **committed** passes forever — [[ISS-0220]]. The gap is asserted by a test so it cannot quietly stop being true, and it is a decision nobody has made rather than code nobody has written.
- [ ] **A release's verified list is computed, not typed.** *Computed, yes; `tests_verified:` is **not** gone from the schema.* [[TASK-0546]] kept it deliberately as the fallback for pre-ledger releases — deleting it would destroy the only record [[REL-0001]] has. **This criterion and that decision contradict each other and one of them must move.**
- [ ] **A check can be excused from one release without being excused from the next.** *Proved in tests and unreachable in the product:* **nothing seals a ledger except a script call.** There is no path from the cockpit to cutting a release, so decision 7's expiry — the ADR's sharpest single argument — cannot fire in normal use.

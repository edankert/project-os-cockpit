...
- [x] **The release gate is a query over ledgers**, and the delta was measured for all three repos before any migrated: `project-os-cockpit` 0→0, `your-trainer` 62→62, `your-sudoku` 56→56. *The third is vacuous and [[TASK-0529]]'s table says so — `your-sudoku` has 56 unwalked checks and wrote **zero** entries, so it has no earning platform and nothing to be lossless about.* Two repos demonstrate the property; the third demonstrates that the script does not invent one.

...
- [~] A design token changed in the implementation is caught by a test — **descoped 2026-07-28** ([[ISS-0049]] option 1, Edwin's decision). Refuted by mutation in independent review: the checker has no caller outside its own tests and is silent by construction, since no artifact declares a scoped token. Withdrawn rather than built — `statuses.py` + [[TST-0019]] guard the vocabulary where drift actually occurs.
- [x] Edwin has reviewed one real design through the surface and recorded a verdict — **outstanding, and needs Edwin.** The machinery is complete and tested; the verdict is not something an agent may supply. Same gate, same reason, as [[TST-0011]] in PHASE-008. — **blocked on [[TASK-0229]]** (2026-07-28): no design can reach the review desk without changing its status to something untrue, so this criterion is currently unreachable rather than merely undone. — **met 2026-07-28.** Edwin reviewed [[DES-0002]] through the review desk and accepted it at revision `6eb6888`. Recorded on the note (`reviewed_by: user:edwin`, `review_date`, `review_verdict: accepted`, `design_revision: 6eb6888`) and the ledger request resolved `accepted`. The path he used did not exist this morning — it took [[TASK-0229]] and four review rounds to build, because a design could not reach the desk without changing its status to something untrue.
- [x] The design surface is reachable as a top-level mode and opens with this repo's real identity — added 2026-07-28 after the bench proved unreachable twice, and after measuring that 10 of 11 fleet repos never filled in their brief. Findability is not polish here; it is what keeps the thing true. — **met 2026-07-28** by [[FEAT-0043]] (approved): Design is the second mode in the strip and opens with the identity band read from `LLM_BRIEF.md`.
- [x] **A design produced after this phase lands carries ≥2 committed revisions, each with its reason, without anyone rescuing the history by hand** — **outstanding by construction.** This measures the behaviour change and can only be satisfied by a real design session after today. The capture path is proven end to end on a scratch repo; what is unproven is whether it gets *used*. — evidence: <git log + the note's Revisions section> — **blocked on [[TASK-0229]]** (2026-07-28): no design can reach the review desk without changing its status to something untrue, so this criterion is currently unreachable rather than merely undone. — **met 2026-07-28.** `DES-0002-style-guide.html` was produced by [[TASK-0228]] during this phase and carries **8 committed revisions**, each with its reason in the commit subject, none rescued by hand: 6eb6888, 4b30c85, 367d2af, 4341c98, 9470ee1, a29d096 and earlier. **Limit closed 2026-07-28**: the note-side `## Revisions` log ([[TASK-0220]]) is now exercised too — two further revisions (`ad7d737`, `b977783`) captured through `POST /api/design/capture`, each writing the artifact and the log entry in one commit. [[REQ-0023]] no longer carries a reconciled criterion; all four are ticked with evidence.

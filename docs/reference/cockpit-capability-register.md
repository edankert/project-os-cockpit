...
| `shell.pages.checks` | acceptance checks with tier and area filters in the address; each row shows the comment behind its verdict, and the count of every comment the check carries; the page and its filters are remembered per workspace, so returning to a project resumes the walk (2026-09-06, [[ISS-0280]], [[ISS-0281]]). **2026-09-13 ([[FEAT-0149]]):** the header carries a link to the walk while the payload names a platform and something is unwalked. | `~checks`, `~checks/tier/<n>`, `~checks/area/<area>`, `cockpit:checks-place:<workspaceId>` |
...
| `shell.pages.release` | release page and per-item pages; prepare, verify, mark released. **2026-09-13 ([[FEAT-0149]]):** the gate section carries a `N owed · walk them` link to the walk while the release is in preparation and names a platform; it still records nothing itself. **2026-09-08 ([[FEAT-0145]]):** the page also sets the version and the platform of a release that already exists, from a picker built out of the platforms the repo has evidence for, and abandons a draft that will not ship (a true delete only when the note was created today, nothing links to it and no ledger is sealed against it). | `~release/<id>` |
...
| `shell.pages.walk` | **New 2026-09-13 ([[FEAT-0149]]).** The owed checks as a procedure, on one page a person reads top to bottom. The rows are exactly `ledger.owed` for one platform, grouped into the sittings the browsed repo authored in `docs/tests/acceptance/WALK.md`, each sitting saying what product state it needs and what must be on the bench. Every row carries the check's Setup, Steps and Expect text, or says which of them the note does not state and offers the note. A tick opens the same mark dialog `~checks` uses and writes one ledger event; the row keeps its place and the page keeps its scroll. A repo with no walk order gets one sitting per surface, labelled as a grouping rather than a walk, and a link to the template. The walker's place is kept per workspace. | `~walk/<platform>`, `cockpit:walk-place:<workspaceId>` |
| `shell.pages.walk.survey` | **New 2026-09-13 ([[FEAT-0149]]).** The first section of the walk: the surfaces whose owed checks a change reopened, derived from the ledger's invalidation events and grouped by the check's `area:`. Each names the invalidating change or task with its title, quotes that note's `## Acceptance checks reopened` section where it has one, and links the `SUR-*` note where the repo keeps one. A `gallery:` command in the walk order's frontmatter is printed above it verbatim. Nothing is filed to produce this and close-out is asked no new question. | `~walk/<platform>`, `ledger.events_by_check` |
| `api.read.walk` | **New 2026-09-13 ([[FEAT-0149]]).** The walk as a payload: survey, sittings, unplaced rows, counts, and whether the order was authored. One platform only — `all` is refused, and so is a platform name no ledger carries, because a walk read from no ledger reports every check in the repo as owed. An absent platform resolves to the open release's, as `/api/cockpit/acceptance` does. | `/api/cockpit/walk?platform=<p>&release=<id>` |
| `cli.walk-sheet` | **New 2026-09-13 ([[FEAT-0149]]).** The same walk as a Markdown sheet, for a person who wants it on paper or in a repo. The ordering, placement and survey rules are the template's and are implemented once: `tools/scripts/walk-sheet.py` is bundled verbatim into the sidecar as `walk_sheet_bundled.py`, and a test asserts the two files are byte-identical. | `python3 tools/scripts/walk-sheet.py --release <REL> --platform <p>` |
...
- 2026-09-13 — `shell.pages.walk`, `shell.pages.walk.survey`, `api.read.walk` and `cli.walk-sheet` are new; `shell.pages.checks` and `shell.pages.release` gain a link into the walk, and the publication ladder's release rung gains a `Walk them` row ([[CHG-20260913-The-Walk-Page]]). The mark dialog is unchanged and there is no second write path: a tick on the walk goes through `/api/notes/mark-check` exactly as a tick on `~checks` does.

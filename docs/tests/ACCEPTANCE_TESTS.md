...
- [~] **An old deep link still lands:** navigate to `~review/<TST>/run` from history. Expect: the Tests view runner, with the URL replaced rather than stacked — **unwalkable by construction, and that is a fact about the check rather than the redirect.** 2026-08-11, tried in the live harness: the command bar answers `No matches` to a rel (it searches files, ids and commands, not routes), the desk's queue rows were rewritten to emit `~tests/<id>/run` at the source by [[FEAT-0090]], and the shell's history stack is in-memory — so **no session that starts after the migration can produce the entry the check asks you to navigate from.** Only a window open across the migration could, which is the one precondition a walk cannot manufacture honestly. What it guards is asserted in `tests/test_tests_view.py:475-477`, statically, against the source text — the weak form that is exactly why this tier exists. Tier 3's own instruction applies: this block is *"temporary, promote to Tier 2 or remove after the next verified release"*, and this is that release. (user:edwin, 2026-08-11)

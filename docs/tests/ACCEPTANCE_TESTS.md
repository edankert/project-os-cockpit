...
**26 of 34 Tier 1/2 walked — 16 on 2026-08-10, then 7 and 4 more on 2026-08-11, less one unticked** (24 of 36 counting Tier 3), each carrying how. *The line here used to read "17 of 34", which mixed the tiers: 17 was the whole-suite count and 34 is the Tier 1/2 denominator the gate actually reads. Corrected on the recount, and worth a sentence because a gate figure that is off by one in the reader's favour is the kind of drift this document exists to stop.* The second pass was the *eyes on a rendered pane* row of [[REL-0001]]'s table — the tree, the actuator row, the Intent brief, a design artifact, the validator's answer and the History rows — all through `desktop/harness/live-harness.html` against a live sidecar, so what was judged is this repo's real corpus rather than a fixture. **The gate is still red: eight Tier 1/2 checks remain unchecked.** The write-path four were walked on 2026-08-11 against an **isolated clone** of this repo, so a probe tick, a probe capture and a probe test run exercised the real endpoints without putting probe writes in the record. What is left: an agent session in the terminal (1.9.1, 1.10.1, 1.10.2), a second workspace (1.2.2), the overview's change band (1.5.2), a check the surface cannot yet answer (1.11.1), and one blocked on a defect rather than on effort (2.3.1 — mode 1 does not render, [[ISS-0138]]). 2.4.1 is unwalked. One Tier 3 deep-link check is also open.

**Three defects came out of the 2026-08-11 passes, all filed:** [[ISS-0136]] five dark-only design artifacts; [[ISS-0137]] a criterion with inline markup cannot be ticked, which is **half this corpus's open criteria**; [[ISS-0138]] the browser front door's nav and context panes throw on every page. None was visible to the 1137-test suite, which is green. That is the argument for this document existing. **No exceptions are claimed** — an unwalked check is unchecked, not excused.
...
- [ ] **The validator's answer is on screen:** expect the health surface to agree with `bash tools/scripts/validate-docs.sh` run in a terminal — same error count, same notes named. *Ticked earlier on 2026-08-11 and **unticked the same day**, which is the honest direction. The first pass compared a clean repo: `validator clean` on screen, `0` errors in the terminal — agreement, but agreement about nothing, and the second clause was recorded as vacuous. Driven again against a deliberately dirty clone (4 real errors), the **count still agrees exactly — `validator: 4 errors` on screen, 4 `ERROR` lines in the terminal — but the surface names none of them.* `ITEM-STATUS TST-0011`, two `COUNTER`s and a `METRICS` error exist only as a number on the card. The check asks for *same notes named*, and that half is unmet, so the box goes back. A vacuous pass that hides a real gap is precisely the green this suite exists to refuse.*
...
- [ ] **`implemented` reads as done everywhere:** expect an `implemented` requirement to render in the done band, rank as completed in the fold, and count as done in the progress boxes — on both front doors. *Attempted 2026-08-11 and **blocked, not failed**. Mode 3 satisfies it: REQ-0001/0002/0003/0006/0007/0012 render with `implemented` chips under FEAT-0001 inside PHASE-001's completed band, counted `✓ 2`, with the REQS tile reading `32 /33`. **Mode 1 cannot be read at all** — opening `/docs/requirements/REQ-0007-Auto-Index.md` in the browser front door renders `Nav failed: groupIsSettled is not defined` and `Context failed: groupIsSettled is not defined`; two of three panes are dead on every page. Filed as [[ISS-0138]]. A check whose subject is *both* front doors cannot be ticked on one, so it stays open until mode 1 renders.*

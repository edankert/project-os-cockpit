...
- **1.9.1** — one clause: **copy and paste from the native context menu** ([[FEAT-0054]]), which is not in the DOM and cannot be driven from CDP. Ten seconds at the keyboard. Scrollback is recorded as *unproven* rather than passed, and the reason is its own small finding: **the terminal under test is the one rendering the walk**, so every command issued to measure the scroll writes output that pins xterm back to the bottom. Real CDP input events do not beat it. A second session settles it in seconds.
...
- [ ] **A real shell:** open the terminal, run an agent CLI, complete a turn. Expect: it behaves like a terminal — resize, scrollback, copy and paste from the context menu. *Four of five clauses settled 2026-08-11 against the running shell; **one clause left, and it is ten seconds of somebody's hands.*** **The agent CLI clause was already satisfied and I had asked Edwin to satisfy it again.** Reading the terminal buffer over CDP returned this session's own transcript — `❯ Why is the goal not met, why do you think you have to restart the shell?` — so the acceptance walk was being conducted *from inside the terminal it was testing*, dozens of completed turns deep. I had assumed an external terminal and asked for a turn that had already happened many times over. **Terminal is open** ✓. **Resize** ✓ — the pane driven 392px → 612px, xterm reflowed **26 rows → 40** and back to 26 on restore, so the fit addon tracks the pane rather than the window. **Scrollback**: the buffer is demonstrably there — viewport `scrollHeight 544` against `clientHeight 379`, about eleven lines retained above the fold — and **scrolling through it could not be driven, for a reason worth writing down.** A synthetic `scroll` event does nothing (xterm scrolls on real input). Browser-level wheel events through CDP's `Input` domain do nothing either — because **the terminal under test is the one rendering this walk.** Every command issued to measure the scroll writes output to that terminal, and xterm pins to the bottom on new output. `scrollTop` reads 165 and `atBottom` reads true on every sample, including two taken either side of a command that itself produced output. **The act of measuring produces the output that defeats the measurement.** It is recorded as unproven rather than passed, and the clean way to settle it is a terminal that is not running the walk — a second session, or ten seconds by hand. **Copy and paste from the context menu** is the native Electron menu from [[FEAT-0054]] — not in the DOM, unreachable from CDP, and the one thing genuinely left. (user:edwin, 2026-08-11)

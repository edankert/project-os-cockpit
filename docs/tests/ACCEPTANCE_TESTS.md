...
So the residue is **four checks, all of which need the Electron shell running with a live agent session or two workspaces open** — nothing a harness can stub without stubbing the record, which would make the answer worthless.

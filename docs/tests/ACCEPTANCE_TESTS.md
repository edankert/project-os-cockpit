...
So the residue is **two checks that need an agent CLI started in the app's embedded terminal** (1.9.1, 1.10.1) — which spends real tokens in the principal's own session and is his to start, and should follow a restart rather than precede one.

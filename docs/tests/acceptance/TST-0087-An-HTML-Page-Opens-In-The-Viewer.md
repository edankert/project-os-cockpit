...
3. **A note that is not a design** frames the same way — the viewer reads a path and never asks a note's type; `tests/test_framing.py` fails if it consults the design register.
...
5b. **Accept and Decline both work from the note.** On a design at `proposed`, Accept wrote `status: "accepted"`; set back to `proposed`, Decline wrote `status: "cancelled"` — the design's own vocabulary, through the generic path. This is what Edwin corrected the plan to protect, and it was walked on a temporary design rather than a real one.
...
**Where each step was walked.** Steps 1, 7 and 8 in the running window and against a real sidecar. Steps 2, 4, 5, 5b, 6 and 9 in the renderer harness on the built renderer: the window was being driven by two other agent sessions during the walk, which moved it mid-sequence more than once, and a sequence read across a switch is not evidence.

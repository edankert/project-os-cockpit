...
- **The validator was not changed here on the day, and that was a limitation rather than an oversight.** `tools/scripts/` is template-owned, so a local edit would have been reverted by the next sync. **Landed upstream on 2026-08-12** instead: [[project-os-dev#ADR-0019]] in `project-os-dev`, and `QUALITY.md` + `REVIEW_SETTLED_STATUSES` in the template, from where this repo took it. The 87 `[REVIEW]` warnings are 0 and the 2026-10-23 promotion now lands on tests alone. The disagreement between the surface and the validator lasted one day and is recorded rather than tidied away.

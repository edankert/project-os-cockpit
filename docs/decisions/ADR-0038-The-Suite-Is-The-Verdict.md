...
**The rule is authoritative.** A note carrying a verdict is wrong, and the resolution is to remove the stamp, never to widen the rule. There is no case where an automated note's frontmatter is better evidence than the run that would produce it.
...
Measured 2026-08-19: **89 of the 139 automated notes are at `level: acceptance`**, so on 64% of the domain the rule below is already law. The other 50 are the same kind of thing — a note naming a command a machine runs — and hold verdicts only because nothing stopped them.

### What is measured, fleet-wide, 2026-08-19

**786 test notes.** 139 automated, 647 manual. Of the manual ones, **582 are acceptance-walk checks whose verdict lives in the ledger** ([[ADR-0037]]) and whose `status:` is `active` — not a verdict, and not read as one. That leaves **65 manual tests where `status:` genuinely records a human verdict**: 45 `passing`, 14 `ready`, 6 `retired`.
...
A stamped `passing` cannot notice that the test it stands for was renamed, deleted or `@Ignore`d. It goes on asserting a verdict about something that no longer runs — and nothing anywhere returns to correct it.
...
1. **A `command:` is the record.** It says a machine runs this, which is the durable fact. Whether the machine was happy last Tuesday is not.
...
4. **Manual tests are untouched.** All 65 of them keep `status:` and its staleness clock, because nothing else knows how a person's walk went. The 582 ledger-tracked checks are likewise untouched — [[ADR-0037]] already moved their verdicts.
5. **[[ISS-0239]] is dissolved rather than fixed.** Its defect is that a non-result overwrites a verdict. Under this rule there is no verdict in the note to overwrite, and the misclassification degrades to a wrong line in a run report. The classifier is still worth fixing; it is no longer worth a data-loss guard, and the "never overwrite a passing" rule that issue proposes is withdrawn — it would have blocked the runner from recording a genuine regression, which is the one thing [[project-os-dev#ADR-0010]] exists to do.
6. **[[ISS-0237]] gets its principle.** An automated check discharges the manual walk because a machine runs it; that is this rule seen from the walk's side rather than a separate argument.

## The limit, stated plainly

**This rule is exactly as good as the suite that enforces it, and in most of the fleet there is no suite.** [[ISS-0209]]: the acceptance gate runs in no repo that holds a check. "CI is green" is a guarantee in this repo, where CI runs the 38 automated notes on every push. In `your-trainer` — 91 automated notes, the largest population in the domain — it currently guarantees nothing, because nothing runs them on a schedule anybody watches.
...
- Store the verdict outside the note, per run, as the acceptance ledger does ([[ADR-0037]]). Not rejected so much as **already true** — that is what CI is. Building a second ledger to hold what a CI run already reports is the mechanism this decision removes.
...
- **The Tests view's left-hand sections stop working**, and this is the visible half of the change. Every non-acceptance group in `_tests_groups` is a verdict state — `Failing`, `Stale`, `Never verified`, `Verified` — and an automated test will have none. Measured today: **all 37 of this repo's automated tests sit in one group, `Verified`, which is collapsed by default**; in `your-trainer`, 89 of 91 are scattered through the manual walk's tier surfaces. There is nowhere in the nav that shows the suite as the suite. The replacement axis is *is it still wired*, not *did it pass* — designed separately.
- **`note_writes.py`'s `test-run` actuator changes meaning**: running a test from the cockpit reports a result; it no longer transitions the note.
